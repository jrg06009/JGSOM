  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Batting */}
        {['away_batting', 'home_batting'].map((key, i) => {
          const team = key === 'away_batting' ? box.away : box.home;
          return (
            <div key={key}>
              <h2 className="text-lg font-semibold mb-1">{team} Batting</h2>
              {box[key]?.length ? (
        ...
        {/* Pitching */}
        {['away_pitching', 'home_pitching'].map((key, i) => {
          const team = key === 'away_pitching' ? box.away : box.home;
          return (
            <div key={key}>
              <h2 className="text-lg font-semibold mb-1">{team} Pitching</h2>
              {box[key]?.length ? (
                <table className="text-sm border w-full mb-2">
                  <thead className="bg-gray-100">
